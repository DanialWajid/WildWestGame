...
public class PlayerShoot : MonoBehaviour
{
    ...
    public GameObject gun; // Drag your M9 Gun object here
    ...
    [Header("Crosshair Anims (Legacy)")]
    public Animation topHair, bottomHair, leftHair, rightHair;

    private Reaload reloadScript;
    private PlayerHealth health;

    void Start()
    {
        reloadScript = GetComponent<Reaload>();
        health = GetComponent<PlayerHealth>();
        UpdateUI();
    }

    // LateUpdate runs AFTER the animation system has moved the bones
    void LateUpdate()
    {
        if (health != null && health.currentHealth <= 0) return;
        // This forces the gun to stay in the hand correctly even during animations
        if (gun != null)
        {
    ...
    public void Shoot()
    {
        if (bulletsLeft <= 0 || (reloadScript != null && reloadScript.isReloading))
        {
            return;
        }
        ...
        if (aimRecoilAnimation != null)
        {
            aimRecoilAnimation.Rewind(); // Forces the recoil to start from the beginning every shot
            aimRecoilAnimation.Play();
        }

        // Play Animations
        if (characterAnimation != null)
        {
            characterAnimation.Rewind("Shoot");
            characterAnimation.Play("Shoot");
        }

        PlayCrosshairAnim();

        // Force gun position immediately on shot
        FixGunTransform();
        ...
        if (Physics.Raycast(ray, out hit, range))
        {
            ...
            if (enemy != null)
            {
                if (hit.transform.CompareTag("EnemyHead"))
                    killCamScript.StartKillCam(hit.point, enemy);
                else
    ...
    void SpawnPlayerTracer(Vector3 start, Vector3 end)
    {
        ...
        float lifeTime = (Time.timeScale < 1f) ? 0.4f : 0.1f;
